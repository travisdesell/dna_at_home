...
int getStartPosition(int *existing_pos_, int sequence_length_, int motif_length_, int number_motifs_)
{
   int pos_exists = 1;
   int rand_num = 0;
   int i;
   while(pos_exists)
   {
      rand_num = (int)(dsfmt_gv_genrand_open_close() * (sequence_length_ - motif_length_));
      for(i=0;i<number_motifs_;i++) 
      if (((rand_num >= existing_pos_[i]) && (rand_num < (existing_pos_[i] + motif_length_))) || ((rand_num <= existing_pos_[i]) && ((rand_num + motif_length_) > existing_pos_[i] )))
         {
            pos_exists = 1;
            break;
         }
...
int main(int number_arguments, char **arguments) {
   int character, i, j, k, offset;
   char **sequences, **motifs;
   int **start_position;
   float **probabilities, randNum;
   int number_sequences, sequence_length,palindrome_length, insert_all_motifs, total_motifs;
...
*/
   ...
   {
      total_motifs= number_sequences*number_motifs;
      motifs = (char**)malloc(total_motifs * sizeof(char*));
      for (i = 0; i < total_motifs; i++) motifs[i] = (char*)malloc((motif_length + 1) * sizeof(char));
   }
   else
   {
...
// printf("overrepresented sequence: [%s]\n", motifs);
   ...
   // if(i<5)  character=i+1; else character=0; //For progression

      if(MODEL_BUILD_PROBABILISTIC==motif_build && 0 !=i)
            offset+=number_motifs;
      else
            offset=0;

      for (j=0; j < character; j++) {
         start_position[i][j] = getStartPosition(start_position[i], sequence_length, motif_length, number_motifs);
      if(MODEL_TYPE_REVERSE == motif_model)
      {

         for (k = 0; k < motif_length; k++) {
            if(motifs[j+offset][motif_length-k-1] == 'A')
               sequences[i][start_position[i][j] + k] = 'T';
            if(motifs[j+offset][motif_length-k-1]=='T')
               sequences[i][start_position[i][j] + k] = 'A';
            if(motifs[j+offset][motif_length-k-1]=='G')
               sequences[i][start_position[i][j] + k] = 'C';
            if(motifs[j+offset][motif_length-k-1]=='C')
      ...
      for (k=0; k < motif_length; k++)
            sequences[i][start_position[i][j] + k] = motifs[j+offset][k];
      }
      }
   }
